...
Pose::Pose(std::string configuration)
{

#if (defined(VISP_HAVE_OPENCV_NONFREE) || defined(VISP_HAVE_OPENCV_XFEATURES2D))
    this->configurationFile = "detection-config-SIFT.xml";
#endif

    this->keypoint_detector.loadConfigFile(configurationFile);
    this->keypoint_detector.loadLearningData("teabox_learning_data.bin", true);
}
...
void Pose::buildReference(vpImage<unsigned char> &I)
{
